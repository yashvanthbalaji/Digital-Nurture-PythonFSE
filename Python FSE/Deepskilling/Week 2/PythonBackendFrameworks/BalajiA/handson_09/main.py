...
# CORS Configuration — allows frontend at localhost:3000 to call this API
...
# ── JWT DEPENDENCY ───────────────────────────────────────────
# Add to any route: current_user: models.User = Depends(get_current_user)
# FastAPI will call this automatically and return 401 if token is bad
...
#  AUTH ENDPOINTS — /api/v1/auth/  -BY BALAJI A

# OAUTH2 COMMENT (Step 95) 
# Simple JWT Login (what we built here):
#   User sends email+password to our /login/
#   We verify against OUR database and return a JWT
#   Use for: users who registered on YOUR platform
#
# OAuth2 Authorization Code Flow ("Login with Google"):
#   1. User clicks "Login with Google"
#   2. Browser redirects to Google consent screen
#   3. User approves → Google sends us an auth CODE
#   4. Our server exchanges CODE for access token (server-to-server)
#   5. We call Google API to get user info
#   User NEVER gives us their Google password — key difference!
#   Use for: third-party login / social login
...
@app.post(
    ...
    status_code=201,
    tags=['Authentication'],
    summary='Register user — stores only bcrypt hash, never plain password'
)
async def register(user_data: schemas.UserRegister, db: AsyncSession = Depends(get_db)):
    ...
    # Hash password — plain text is NEVER saved anywhere
...
@app.post(
    ...
    tags=['Authentication'],
    summary='Login — returns JWT access token valid for 30 minutes'
)
async def login(login_data: schemas.LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    user = result.scalar_one_or_none()
    # Return SAME error for wrong email AND wrong password
